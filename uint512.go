// uint512: Fixed size 256-bit math library
...
// Int is represented as an array of 8 uint64, in little-endian order,
// so that Int[7] is the most significant, and Int[0] is the least significant
type Int [8]uint64
...
// If buf is larger than 64 bytes, the last 64 bytes is used. This operation
...
func (z *Int) AddUint64(x *Int, y uint64) *Int {
	...
	z[4], carry = bits.Add64(x[4], 0, carry)
	z[5], carry = bits.Add64(x[5], 0, carry)
	z[6], carry = bits.Add64(x[6], 0, carry)
	z[7], _ = bits.Add64(x[7], 0, carry)
	return z
}
